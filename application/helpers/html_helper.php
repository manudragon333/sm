    ...
    if (!empty($messageArr)) {
        ...
        foreach ($messageArr as $k => $v) {
            $result.='<li>
                        ...
                        ' . $v->message . '<div class="post_date p_a">' . ((!empty($v->date_added)) ? '<span>Posted on: </span> '.dateFormat($v->date_added, 'd-M-y') : '') . ' </div>
                    </li>';
        }
    } else {
        $result.='<li>
                    <img src="' . base_url() . '/css/images/pushpin_pink.png" class="upd_std" alt="user">
                    No Messages<div class="post_date p_a">' . ((!empty($v->date_added)) ? '<span>Posted on: </span> '.dateFormat($v->date_added, 'd-M-y') : '') . ' </div>
                </li>';
    }
    $result.=' </ul>';
    return $result;
...
function load_checkboxes($table, $name, $extraAttrs = '', $selected_val = array(), $where = array('status' => '1')) {
    ...
    //print_r($data); return true;
    foreach ($data as $k => $v) {
        $html.="<label><input type='checkbox' value='$v->id' name='$name' $extraAttrs ";
        if (in_array($v->id, $selected_val)) {
