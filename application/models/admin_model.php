...
//            redirect('admin/user_accounts');
    ...
    function getStaffPeriods($staff_id = 0, $cycle_id = 0, $academic_year_id = 0) {
        $sql = "SELECT scp.*,sr.name AS staff_name, pc.name AS cycle_name, p.time_label, s.name AS subject_name,
                    c.name AS course_name, b.name AS branch_name, sem.name AS sem_name, sec.section AS section_name
                   ...
                   WHERE (scp.user_id='$staff_id' || 
                        scp.id IN (
                                SELECT lwa.period_id
                                    FROM leave_work_adjusts AS lwa
                                    LEFT JOIN leave_letters AS ll ON ll.id=lwa.leave_letter_id
                                    WHERE ll.is_approved='1' AND lwa.work_adjusted_to='$staff_id'
                            )
                        ) 
                       AND scp.cycle_id='$cycle_id' 
                       AND scp.academic_year_id='$academic_year_id' AND scp.status='1'
    ...
    function checkAttendanceCollision($post) {
        $sql = "select * from staff_cycles_periods
                where  cycle_id='" . $post['cycle_id'] . "' and weekday_id='" . $post['weekday_id'] . "' and period_id='" . $post['period_id'] . "' and subject_id='" . $post['subject_id'] . "' and user_id!='" . $post['user_id'] . "' " . ((!empty($post['section_id'])) ? " AND section_id!='" . $post['section_id'] . "' " : "") . " and status='1' "; // academic_year_id='".$post['academic_year_id']."' and
        $res = $this->db->query($sql);
        return $res->num_rows();
    }
...
//        echo '<pre>'; print_r($userDetails);echo '</pre>';
    ...
    function get_academic_calendar_details($id = 0) {
        if ($id) {
            ...
            $sql = "
                        SELECT a.*,s.name AS sem_name,b.name AS branch_name ,s.sem_number
            ...
            $sql = "
                        SELECT a.course_id
                            FROM academic_calendar_courses AS a
                            WHERE a.calendar_id=$id
                    ";
            $res = $this->db->query($sql);
            $return_data['calendar_courses'] = $res->result();
            return $return_data;
        }
    }
    ...
    function clear_calendar_course_ids($id = 0) {
        if (!empty($id)) {
            $sql = "
                    DELETE FROM academic_calendar_courses 
                    WHERE calendar_id=$id
            ";
            return $this->db->query($sql);
        }
    ...
    function get_students($college_id = 0, $course_id = 0, $branch_id = 0, $semester_id = 0, $section_id = 0) {
        $sql = '
                    SELECT sr.*
                    FROM student_records AS sr
                    LEFT JOIN student_semisters AS ss ON (ss.user_id=sr.user_id AND ss.is_current="1")
                    ...
                    LEFT JOIN branches AS b ON b.id=sr.branch_id
                    LEFT JOIN sections AS sec ON sec.id=sr.section_id
                    WHERE 1 
                ';
        if (!empty($college_id)) {
        ...
        if (!empty($semester_id)) {
            $sql.=' AND ss.semister_id="' . $semester_id . '" ';
        }
    ...
    function upgrade_students_sem_sec($user_ids = array(), $semester_id = 0, $section_id = 0) {
        if (!empty($user_ids)) {
            $userIds = implode(',', $user_ids);
            $oldStudentSemestersIDs = array(); // IDS For updating is_current=0 at later stage(end of func)

            /*
             * Fetch the current student_semisters records that needs to be updated is_current=0 
             */
            $ss_sql = '
                        SELECT ss.id
                        FROM student_semisters ss
                                LEFT JOIN student_records AS sr ON sr.user_id=ss.user_id
                                WHERE ss.is_current="1" AND ss.user_id IN (' . $userIds . ')
                ';
            $ss_res = $this->db->query($ss_sql);
            $oldStudentSemesters = $ss_res->result();
            if (!empty($oldStudentSemesters))
                foreach ($oldStudentSemesters as $k => $v) {
                    $oldStudentSemestersIDs[] = $v->id;
                }
            $oldStudentSemestersIDs = implode(',', $oldStudentSemestersIDs);

            /*
             * Update by iniserting the semister id below Query for each student.
             */
            $sql = '
                        INSERT INTO student_semisters (user_id,semister_id,is_current) (
                                SELECT ss.user_id, ' . $semester_id . ' ,"1"
                                        FROM student_semisters ss
                                        ...
                                        AND ss.user_id IN (' . $userIds . ')
                            )
                    ';
            $this->db->query($sql);

            /*
             * Updated the section id of the students
             */
            $sql_2 = 'UPDATE student_records AS sr
            ...
            /*
             * Updated the is_current to 0 for Old records 
             */
            $sql_3 = 'UPDATE student_semisters ss
                                SET ss.is_current="0"
                                WHERE ss.is_current="1"
                                AND  ss.id IN (' . $oldStudentSemestersIDs . ')
                        ';
    ...
    function clear_staff_attendance($id = 0) {
        if (!empty($id)) {
            echo $sql = "
