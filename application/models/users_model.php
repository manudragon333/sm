...
class Users_model extends CI_Model {
    ...
    function get_user_email_mobile($user_id = 0, $user_type_id = 0) {
        ...
        if ($user_type_id == 0) {
            $user_details = $this->user_details($user_id); /* print_r($user_details); */
            if (!empty($user_details))
                $user_type_id = $user_details[0]->users_type_id;
        }
    ...
    function confirm_office_pass($pass = '', $user_id = 0) {
        $sql_OTHER_DEVELOPER = "select u.id from users as u left join users_type as ut on u.users_type_id=ut.id where u.username='office' and u.password='" . $pass . "' and u.status='1'";
        $sql = "SELECT * 
                    FROM users AS u 
                    LEFT JOIN users_type AS ut ON u.users_type_id=ut.id
                    WHERE u.id='$user_id' and u.password='" . $pass . "' and u.status='1'
        ";
        $res = $this->db->query($sql);
        if ($res->num_rows() == 1)
            return true;
    ...
    public function get_user_notifications($user_id = 0) {
        $sql = "
                SELECT n.*, u.users_type_id
                    FROM notification_panel AS n
                    LEFT JOIN users AS u ON u.id=n.created_by
