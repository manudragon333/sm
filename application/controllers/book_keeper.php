...
class Book_keeper extends CI_Controller {
    ...
    function __construct() {
        ...
        parent::__construct();
        $this->load->model(array('students_model', 'library_model', 'users_model'));
    ...
    function books_dispatched_grid() {
        if ($this->input->post()) {
            ...
            if (isset($post['book_number']) && !empty($post['book_number']))
                $sql.=" and l.unique_number like '%" . $post['book_number'] . "%'";
            if (isset($post['student_number']) && !empty($post['student_number']))
                $sql.=" and sr.students_number like '%" . $post['student_number'] . "%'";
            if (isset($post['staff_number']) && !empty($post['staff_number']))
                $sql.=" and str.code like '%" . $post['staff_number'] . "%'";
            // echo $sql;
    ...
    function receive_books_grid() {
        if ($this->input->post()) {
            ...
            if (!empty($post['student_number']))
                $sql.=" and sr.students_number like '%" . $post['student_number'] . "%'";
            if (!empty($post['teacher_number']))
                $sql.=" and str.code like '%" . $post['teacher_number'] . "%'";
            // echo $sql;
