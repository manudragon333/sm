...
class Office extends CI_Controller {
    ...
    var $send_data_email_id = 'rajutsn@gmail.com';
    var $ci_user_details;
    ...
    function no_due_print($id, $option = FALSE) {
        if ($id) {
            ...
            foreach ($nodue_data as $k => $v) {
                if ($v->approver_status == '2' || $v->approver_status == '0') {
                    $all_approved = false;
                }
            }
            if ($all_approved) {
    ...
    function feepayment() {
        ...
        if ($this->input->post()) {
            ...
            $result = $this->users_model->confirm_office_pass($pass, $this->ci_user_details->id);
            if ($result) {
...
############### END CASH BOOK #########################
    ...
    function addvoucher() {
        ...
        $post = $this->input->post();
        $this->load->model('debitvoucher_model');
        if ($post) {
    ...
    function view_voucher() {
        ...
        if ($this->input->post()) {
            ...
            if (!empty($post['vorefno'])) {
                $where .= " AND dv.vorefno = '" . $post['vorefno'] . "' ";
            }
            if (!empty($post['vcreationdate'])) {
                $where .= " AND dv.vcreationdate = '" . $post['vcreationdate'] . "' ";
            }
            ...
            if (count($data->db_data)) {
                ...
                foreach ($data->db_data as $k => $v) {
                    ...
                    $data->rows[$i]['cell'] = array($v['vorefno'], $v['college_code'], $v['vcreationdate'], $v['debitedto'], $v['type'], $v['received'], $v['amount']);
    ...
    function notification_panel() {
        ...
        if (!empty($post)) {
            ...
            redirect('office/notification_panel');
        }else {
            $data['content_page'] = 'office/notification_panel';
            $this->load->view('common/base_template', $data);
        }
    }
