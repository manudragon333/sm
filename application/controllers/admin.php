...
class Admin extends CI_Controller {
    ...
    function __construct() {
        ...
        parent::__construct();
        $this->load->model(array('students_model', 'staff_model', 'admin_model', 'office_model', 'exam_model', 'users_model'));
        $this->request_counts();
    ...
    public function index() {
        ...
        $data["notifications"] = $this->users_model->get_user_notifications($this->ci_user_id);
        $data['content_page'] = 'admin/home';
    ...
    function save_user_account() {  // USING 
        if ($post = $this->input->post()) {
            ...
            if ($post['users_type_id'] == 1) {
                ...
                $studentSemPost=array(
                    'user_id'=>$post['user_id'],
                    'semister_id'=>$post['sem_id']
                );
                $this->general_model->saveRecord($studentSemPost, 'student_semisters ');
    ...
    function load_sms_users_grid() {
        if ($this->input->post()) {
            ...
            $sql = "select sr.*, sm.id as smid,sr.name, sr.students_number,sm.message_type, sm.`status` , sm.sent_to, sm.message
                    ...
                    where users.users_type_id=1 and users.status=1 
                    ";
            if ($post['college_id'] != "")
                $sql.=" and sr.college_id = " . $post['college_id'] . " ";
...
//            $this->load->view('common/base_template',$data);
    ...
    function add_attendance() {
        ...
        if ($post && $post['user_id'] && $post['semister_id']) {
            ...
            if (!isset($data['subject_edit_data'][0]['user_id']))
                $data['subject_edit_data'][0]['user_id'] = $post['user_id'];
            if (!isset($data['subject_edit_data'][0]['semister_id']))
                $data['subject_edit_data'][0]['semister_id'] = $post['semister_id'];
//            $data['content_page']='admin/add_attendance';
    ...
    function save_semester() {
        ...
        if ($post) {
            foreach ($post['semester_names'] as $k => $v) {
                if (!empty($v)) {
                    $post['name'] = $v;
                    $post['sem_number'] = $post['sem_numbers'][$k];
    ...
    function subject_management() {
        ...
        if ($this->input->post()) {
            $post = $this->input->post();
            $sql = " select sub.*,s.name as semister_name, s.year, b.name as branch_name, c.name as course_name, IFNULL(col.name,'-') as college_name 
            ...
            if (!empty($post['college_id'])) {
                $sql.=' AND sub.college_id="' . $post['college_id'] . '" ';
            }
            if (!empty($post['course_id'])) {
                $sql.=' AND sub.course_id="' . $post['course_id'] . '" ';
            }
            if (!empty($post['branch_id'])) {
                $sql.=' AND sub.branch_id="' . $post['branch_id'] . '" ';
            }
            if (!empty($post['semister_id'])) {
                $sql.=' AND sub.semister_id="' . $post['semister_id'] . '" ';
            }
            // echo $sql;
    ...
    function add_subject() {
        ...
        if ($post) {
            ...
            if ((isset($post['subject3'])) && (isset($post['credits3']))) {   //echo $post['subject2'].'-'.$post['credits2'];
    ...
    function getCollegeNumbers($college_id = 0, $course_id = 0, $branch_id = 0, $semister_id = 0) {
        $options = '<option value="">Select</option>';
        $options.=load_select_numbers('student_records', 0, array('users.users_type_id' => '1', 'users.status' => '1', 'student_records.college_id' => $college_id, 'student_records.course_id' => $course_id, 'student_records.branch_id' => $branch_id, 'student_semisters.semister_id' => $semister_id));
    ...
    function getCourseSemesters($college_id = 0) {
        ...
        $sql = "
            SELECT s.*,c.name AS course_name,b.name AS branch_name 
                ...
                LEFT JOIN courses AS c ON c.id=s.course_id
                LEFT JOIN branches AS b ON b.id=s.branch_id
                WHERE s.course_id IN ($course_id)
        ";
        $res = $this->db->query($sql);
        $data = $res->result();
        $options = '';
        foreach ($data as $k => $v) {
            $options.="<option value='$v->id' sem_number='$v->sem_number' ";
            // if($selected_val==$v->id){ $html.=" selected='selected' "; }
            $options.=">$v->course_name - $v->branch_name - $v->name</option>";
        }
        echo $options;
    }
...
//            $user_details=$this->session->userdata('user_details');
    ...
    function get_extend_cycles() {
        $post = $this->input->post();
        if ($post) {
            $data['form_data'] = $post;
            $data['cycle_periods'] = $this->admin_model->getCyclePeriods($post['cycle_id']);
            $this->load->view('admin/get_extend_cycles', $data);
        }
    }

    function clear_staff_attendance() {
        $this->admin_model->clear_staff_attendance($this->input->post('id'));
    }
    ...
    function save_edit_staff_attendance() {
        ...
        if ($this->input->post()) {
            ...
            if ($this->admin_model->checkAttendanceCollision($post)) {
                $msg.= '<br/><div class="error">Subject already allotted for the same period.</div>';
                return true;
            }
            $cycleTimeCollisions = $this->admin_model->checkCycleTimingCollision($post);
            if (!empty($cycleTimeCollisions)) {
                $msg.= '<br/><div class="error">Period Timings might Collide with the other allotted period in the Cycle -- ' . $cycleTimeCollisions[0]['cycle_name'] . ' - ' . $cycleTimeCollisions[0]['period_label'] . ' - ' . $cycleTimeCollisions[0]['time_label'] . '.</div>';
                return true;
            }
            /* echo '<pre>';
              print_r($post); exit; */
            $this->my_db_lib->save_record($post, 'staff_cycles_periods');
            $msg.= '<br/>' . showBigSuccess('Subject saved to period Successfully.');

            if (!empty($post['extend_period_ids'])) {
                $msg.='<br/> <b>Extensions: </b>';
                $extendPost = $post;
                $extendPost['id'] = '';
                foreach ($post['extend_period_ids'] as $k => $v_period_id) {
                    $extendPost['period_id'] = $v_period_id;
                    /*
                     * Copy the same allocation to the period IDs
                     */
                    $period_label = '(' . generalId('period_label', 'periods', 'id', $v_period_id) . ' - ' . generalId('time_label', 'periods', 'id', $v_period_id) . ')';
                    if ($this->admin_model->checkAttendanceCollision($extendPost)) {
                        $msg.= '<br/><div class="error">Subject already allotted for the same period' . $period_label . '.</div>';
                        continue;
                    }
                    $cycleTimeCollisions = $this->admin_model->checkCycleTimingCollision($extendPost);
                    if (!empty($cycleTimeCollisions)) {
                        $msg.= '<br/><div class="error">Period ' . $period_label . ' Timings might Collide with the other allotted period in the Cycle -- ' . $cycleTimeCollisions[0]['cycle_name'] . ' - ' . $cycleTimeCollisions[0]['period_label'] . ' - ' . $cycleTimeCollisions[0]['time_label'] . '.</div>';
                        continue;
                    }
                    $this->my_db_lib->save_record($extendPost, 'staff_cycles_periods');
                    $msg.= '<br/>' . showBigSuccess('Subject saved to period ' . $period_label . ' Successfully.');
                    ;
                }
            }
            ...
            $msg.='<br/>
                    <form action="' . site_url('admin/staff_attendance') . '" method="post">
                        <input type="hidden" name="staff_id" value="' . ((isset($post['user_id'])) ? $post['user_id'] : '') . '" />
                        <input type="hidden" name="cycle_id" value="' . ((isset($post['cycle_id'])) ? $post['cycle_id'] : '') . '" />
                        <input type="hidden" name="academic_year_id" value="' . ((isset($post['academic_year_id'])) ? $post['academic_year_id'] : '') . '" />
                        <input type="submit" name="imageField" id="imageField" class="send button" value="Back" />
                    </form>
                ';
            echo showBigError($msg);
        }
    }
...
//            $user_details=$this->session->userdata('user_details');
    ...
    function edit_academic_calendar($id = 0) {
        ...
        if ($post) {
            ...
            if (!empty($post['course_ids'])) {
                $this->admin_model->clear_calendar_course_ids($calendar_id);
                foreach ($post['course_ids'] as $k => $v) {
                    $savePost = array();
                    $savePost['calendar_id'] = $calendar_id;
                    $savePost['course_id'] = $v;
                    $this->general_model->saveRecord($savePost, 'academic_calendar_courses');
                }
            }
    ...
    /*
     * @@ START: Admin Student semister upgrade to new semisters
     */

    function student_semester_upgrade() {
        $post = $this->input->post();
        if ($post) {
            $data['post_data'] = $post;
            $data['students_data'] = $this->admin_model->get_students($post['college_id'], $post['course_id'], $post['branch_id'], $post['semister_id'], $post['section_id']);
            $data['content_page'] = 'admin/student_semester_upgrade_students_list';
            $this->load->view('common/base_template', $data);
        } else {
            $data['content_page'] = 'admin/student_semester_upgrade';
            $this->load->view('common/base_template', $data);
        }
    }
    ...
    function upgrade_students() {
        $post = $this->input->post();
        if ($post && !empty($post['user_id']) && !empty($post['semister_id']) && !empty($post['section_id'])) {

            $this->admin_model->upgrade_students_sem_sec($post['user_id'], $post['semister_id'], $post['section_id']);

            $this->session->set_flashdata('success_msg', 'Students Successfully Upgraded.');
            redirect('admin/student_semester_upgrade');
        } else {

            $this->session->set_flashdata('error_msg', 'Please select a semester and section to upgrade.');
            redirect('admin/student_semester_upgrade');
        }
    }

    /*
     * @@ END: Admin Student semister upgrade to new semisters
     */
        ...
        if (!empty($post)) {
            ...
            switch ($post['notification_for']) {
                ...
                case 'all':
                    ...
                    $users_3 = $this->exam_model->get_users_of_types('4,5,6,7,8,9,10');
                    $users = array_merge($users_1, $users_2, $users_3);
                    // echo count($users),' - ', count($users_1), ' - ', count($users_2),' - ', count($users_3);
            ...
            redirect('admin/notification_panel');
        }else {
            $data['content_page'] = 'admin/notification_panel';
            $this->load->view('common/base_template', $data);
        }
    }
